      ...
      await pusher.init(
        ...
        onEvent: (event) async {
          ...
          if (event.eventName == "OfferSent") {
            debugPrint("OfferSent event data: ${event.data}");
            await listen(data: event.data);
          }
        },
      );

      await pusher.subscribe(channelName: ' offer.19');
  ...
  Future<void> listen({required dynamic data}) async {
    try {
      ...
      if (data is Map<String, dynamic>) {
        HomeModel tripModel = HomeModel.fromJson(data);
        model = tripModel;
        debugPrint("Model updated from Pusher event.");
        emit(SocketUpdateState());
      } else {
        debugPrint("Event data is not a valid JSON object.");
      }
    } catch (e, stack) {
